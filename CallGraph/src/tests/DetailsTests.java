...
public class DetailsTests {
	...
	public void mutualRecursionTest() {
		try {
			...
			graph = new StringGraph(cg);
			ArrayList<String> edges = graph.nodes.get("samples.SampleTwo.recursaoMutua()I");
			assertTrue(edges.contains("samples.SampleTwo.recursaoMutua()I"));	
		} catch (Exception e) { fail(e.getMessage()); }
	}
	...
	public void indirectRecursionTest(){
		try {
			...
			edges = graph.nodes.get("samples.SampleTwo.recursaoMutuaA()V");
			assertTrue(edges.contains("samples.SampleTwo.recursaoMutuaB()V"));
